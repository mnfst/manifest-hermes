...
If the retry is rejected as well, treat it as a normal error and do not retry again on Manifest's account. MCP tools are repaired over HTTP or stdio, including local MCP servers. Built-in terminal, file, and memory tools are not repaired.

If the user asks why a call was retried, say that Manifest repaired the rejected arguments based on the tool's error.
